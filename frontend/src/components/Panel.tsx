...
import type { RankItem, ActiveBan } from "../types";
...
export function Panel({
  ...
  useLayoutEffect(() => {
    ...
    const updateVisibleCount = () => {
      ...
      const rowHeight = Math.max(...rows.map((row: any) => (row as HTMLElement).offsetHeight));
  ...
  const row = (item: RankItem | ActiveBan, key: string) => (
  ...
  return (
    <section className={wide ? "panel panelWide" : "panel"} ref={panelRef}>
      ...
      <div className="rankList">
        ...
        {visibleItems.map((item) => row(item, (item as any).label))}
      </div>
      ...
      <div className="rankList rankMeasure" ref={measureRef} aria-hidden="true">
        {items.map((item) => row(item, `${(item as any).label}-measure`))}
      </div>
    </section>
  );
}
