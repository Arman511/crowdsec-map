...
export function Panel({ rankings, initialMode, storageKey, wide = false }) {
  const [mode, setMode] = useState(() =>
    readStoredRankMode(storageKey, initialMode),
  );
  ...
  const collapsedLimit = Number.isFinite(visibleCount)
  ...
  useLayoutEffect(() => {
    ...
    const updateVisibleCount = () => {
      ...
      const possible = Math.floor(
        (available + listGap) / (rowHeight + listGap),
      );
      if (
        rows.length * rowHeight + Math.max(0, rows.length - 1) * listGap <=
        available
      )
        setVisibleCount(Number.POSITIVE_INFINITY);
  ...
  const row = (item, key) => (
    <div className={isBanMode ? "rankRow banRow" : "rankRow"} key={key}>
      ...
      {isBanMode ? (
        <em title={item.detail || item.meta || ""}>
          {item.meta || item.detail || "active"}
        </em>
      ) : (
        <>
          <div className="bar">
  ...
  return (
    <section className={wide ? "panel panelWide" : "panel"} ref={panelRef}>
      ...
      <div className="rankList">
        ...
        {visibleItems.map((item) => row(item, item.label))}
      </div>
      ...
      <div className="rankList rankMeasure" ref={measureRef} aria-hidden="true">
        {items.map((item) => row(item, `${item.label}-measure`))}
      </div>
    </section>
  );
}
