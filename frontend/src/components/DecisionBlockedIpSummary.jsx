...
export function DecisionBlockedIpSummary({ total, origins }) {
  return (
    <section
      className="decisionBlockedIpSummary"
      aria-label="Blocked IP address summary"
    >
      <Metric
        icon={<ShieldAlert />}
      ...
      >
        {origins.map((origin) => (
          <div
            className={`decisionOrigin decisionOrigin--${origin.key}`}
            key={origin.key}
          >
        ...
        {!origins.length && (
          <span className="decisionOriginEmpty">No IP decisions</span>
        )}
      </div>
    </section>
  );
