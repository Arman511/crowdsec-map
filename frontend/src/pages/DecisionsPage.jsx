...
export function DecisionsPage({ onSelectIp, refreshSeconds, refreshSignal }) {
    ...
    <section className="decisionsView">
      ...
      <div className="decisionsSummary">
        <Metric
          icon={<ShieldAlert />}
          label="All Decisions"
          value={decisions?.total || 0}
        />
        <Metric
          icon={<Search />}
          label="Matching"
          value={decisions?.matched || 0}
        />
        <Metric
          icon={<Globe2 />}
          label="Countries"
          value={decisions?.countries || 0}
        />
        <Metric
          icon={<Crosshair />}
          label="Scenarios"
          value={decisions?.scenarios || 0}
        />
      </div>
      ...
      <div className="decisionRankingStrip">
        <DecisionRanks
          title="Top scenarios"
          items={decisions?.topScenarios || []}
        />
        <DecisionRanks
          title="Top countries"
          items={decisions?.topCountries || []}
        />
        <DecisionRanks
          title="Top origins"
          items={decisions?.topOrigins || []}
        />
      </div>
      <div className="decisionsTableWrap">
        ...
        {!error && loading && !decisions && (
          <div className="modalLoading">
            Loading CrowdSec enforcement decisions...
          </div>
        )}
        {!error && decisions && (
            ...
            <tbody>
              {decisions.items.map((item) => (
                ...
                >
                  ...
                  <td title={item.until}>
                    {item.duration || item.until || "active"}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <footer className="decisionsPager">
        <span>
          Cached {formatRelativeTime(decisions?.cachedAt)} · 50 per page
        </span>
        <div>
          <button
            type="button"
          ...
          <button
            ...
            onClick={() => setOffset(decisions.nextOffset)}
          >
            Next
          </button>
        </div>
